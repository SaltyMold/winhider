...
Launch `Updater.exe` from the installation directory or command prompt. You'll be dropped into an interactive terminal.
...
```bash
Updater.exe hide 12345
```

Unhide a window from taskbar:
```bash
Updater.exe unhidetask 12345
```
...
1. Run `Updater.exe list` to see all visible windows
...
& "C:\Program Files\WinHider\Updater.exe" hide $notepadWindow.MainWindowHandle
```
